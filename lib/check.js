...
function sgRuleYaml(rule) {
    ...
    if (rule.notInside === undefined && regexes === '')
        return `${header}  ${withText}\n`;
...
function llmFallback(ext) {
    return (`ast-grep covers no ${ext} grammar here (LLM fallback): judge this file against the CORDIS checklist yourself — ` +
...
export async function check(root, options = {}) {
    ...
    for (const abs of files) {
        ...
        if (YAML.has(ext) || SCRIPT.has(ext) || POLYGLOT.has(ext)) {
            ...
            if (options.grammarConfig !== undefined)
                covered.push(abs);
            else
                zig.push(abs);
        }
...
async function listFiles(root) {
    ...
    for (const entry of entries) {
        ...
        const rel = relative(root, abs).split('\\').join('/');
        if (rel.split('/').some(isSkippedDir))
            continue;
        out.push(abs);
    }
    return out;
...
function sgScanAll(files, grammarConfig) {
    if (files.length === 0)
        return [];
...
function sgScanBatch(files, grammarConfig) {
...
function sgIds(rel, byRule, seen = new Map()) {
    const findings = [];
...
async function sgPolyglot(rel, byRule, abs) {
    const text = await readFile(abs, 'utf8');
