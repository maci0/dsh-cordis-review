...
import type { CheckOptions, Finding } from './check.ts'
...
let astGrep: boolean | undefined
let grammarConfig: string | undefined
...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === '--grammar-config') {
    ...
    if (value === undefined || value.startsWith('-')) fail('--grammar-config needs a path')
    grammarConfig = value
    index += 1
    continue
  }
  if (arg.startsWith('-')) fail(`unknown flag ${JSON.stringify(arg)}; try --help`)
  if (root !== undefined) fail(`unexpected extra argument ${JSON.stringify(arg)}; try --help`)
  root = arg
}
...
if (!isDirectory) fail(`not a directory: ${target}`)
