 ...
 */
function splitDocument(source: string): { block: string, body: string, present: boolean } {
...
function readBlockScalar(
  ...
  for (; index < lines.length; index += 1) {
    ...
    if (line.length === indent) {
      ...
      if (index + 1 < lines.length && (lines[index + 1] as string).length >= indent) return undefined
      content.push('')
      continue
    }
 ...
 */
function readScalar(raw: string): { value: unknown } | undefined {
 ...
 */
function parseFlatBlock(block: string): Record<string, unknown> | undefined {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const key = (entry[1] as string).trimEnd()
    if (key !== (entry[1] as string)) return undefined
    ...
    seen.add(key)
    const raw = (entry[2] as string).replace(/^[ \t]+/, '').replace(/[ \t]+$/, '')
    if (raw === '') {
 ...
 */
export async function parseFrontmatterWithYaml(source: string): Promise<Frontmatter> {
  const { block, body, present } = splitDocument(source)
  if (!present) return { data: {}, body }
