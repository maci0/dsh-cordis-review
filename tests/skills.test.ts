...
import type { Frontmatter } from '../src/frontmatter.ts'
 ...
 * dynamically-imported `yaml` parser only when it refuses.
 */
const flatRead = async (source: string): Promise<Frontmatter> => {
  const parsed = parseFrontmatter(source)
  if (parsed !== undefined) return parsed
  const { parseFrontmatterWithYaml } = await import('../src/frontmatter.ts')
...
test('parseFrontmatter reads a literal block scalar and a nested map', async () => {
  ...
  // The nested `provider:` map is outside the flat subset: the fallback ran.
...
test('bundled skills are discoverable and user-invocable', async () => {
  ...
  // Deterministic perf gate: every header this package ships resolves on the
  // flat reader, so serving this catalog never loads `yaml` dynamically.
  for (const name of ['cordis-review', 'cordis-doc-review']) {
    assert.notEqual(
      parseFrontmatter(readFileSync(join(skillsDir, name, 'SKILL.md'), 'utf8')),
      undefined,
      `${name}/SKILL.md fell through to the yaml fallback`,
    )
  }
